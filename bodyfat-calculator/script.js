...
function bfat() {
    ...
    var ibf;
    var bfc;
    if (height == null || height == 0 || neck == null || neck == 0 || waist == null || waist == 0) { $('bf').value = 'Pl. enter data.' } else { $('bf').value = '' }
    if (ms == 'metric' && sex == 'm' && height > 0 && waist > 0 && neck > 0) {
        ...
        if (ibf <= 5) { bfc = 'Essential' } else if (ibf > 5 && ibf <= 17) { bfc = 'Fit' } else if (ibf > 17 && ibf <= 25) { bfc = 'Acceptable' } else if (ibf > 25) { bfc = 'Obese' }
        $('bf').value = ibf + ' % ';
        $('nbf').value = bfc
